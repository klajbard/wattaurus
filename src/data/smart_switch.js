  ...
  content: [
    {
      image: {
        alt: "Sonoff POW R2",
        url: "https://lh3.googleusercontent.com/pw/ACtC-3cNjIQf55TW1S8eFpSvcUiLwQwBZTS92vg5wZX3OvkD1_bfKlKjLPv575qqZK3tap9T9IiaLUhmr20Q0ZqvhZyQITERqF35a_3xc3Q6qUviF03-j_6RlbaFEDCiBmoSNC-cVgYpFqQUizCri47NRCQT=w600-h450-no",
        avoidLazy: true,
        align: "right",
        caption: {
          text: "Sonoff POW R2",
    ...
    {
      text: "If all went well without any error message we are new ready to purge the current firmware and upload the new one. Pay attention to unplug and plug the USB back while pushing the reset button after each operation is done to re-enter the programming mode. The following command will erase the current firmware from the ESP8266. Should take like 3-5 seconds.",
      code: {
        source: "esptool.py -p /dev/ttyUSB0 erase_flash",
      },
    },
    {
      text: "Now the ESP8266 is emtpy, but the device is not in flash mode anymore, so repeat the boot to flash process. Let's grab the latest tasmota firmware from the [GitHub Tasmota Releases](https://github.com/arendst/Tasmota/releases). Choose one you prefer, like one with a custom language, light version or whatever. I'd suggest to go with the simple `tasmota.bin` on the first time as it contains most of the drivers. Once the binary is downloaded run the following command to upload the tasmota firmware. It can take up to 1 or 2 minutes depending on the system.",
      code: {
        source:
    ...
    },
    {
      ...
      image: {
        alt: "Tasmota GUI",
        url: "https://lh3.googleusercontent.com/pw/ACtC-3ctnYF0B3Q65STYo1fKP27j3i8SS8CpubfA2OxOkH026I6RGsHLMJKi-nEFnmy2llkDKTlxKDSYqRkBCk9fJygYRquSuGQTsI6HKuABflH7qOYQnjVtev7iZ7Cula8cDmja1tPLiWPB2mUi6oD-bLuR=w810-h720-no",
        avoidLazy: true,
        align: "right",
        caption: {
          text: "Tasmota GUI",
      ...
      text: "Right after the new firmware is flashed the device will restart and boot in AP mode. This means any device can access the Sonoff switch. Scan your WIFI network and search for a connection named `tasmota-xxxxx`. Join the network (no password is required). It will then ask to sign in to the network and brings to the Tasmota GUI. Here you can provide wifi credentials to the device. Only give it access to 2.4GHz network as it doesn't support 5GHz. You can also provide a secondary network credential if it fails to join the first one. After saving the settings it will reboot. You are free to leave this network and join back to the local network where the device joined.",
    },
    {
      text: "Before accessing the Tasmota GUI on the local network we first need to know it's IP address. There are plenty of ways to do this, but might be the easiest to just type the router IP address (usually `192.168.1.1`) to the browser and check for active connections on the admin panel. Another wasy way - which I prefer - on ubuntu is to run `arp -a` command. It shows all active connections' hostname, IP address, MAC address. Just what I've needed. Once the IP address is identified enter it to the browser. Head to *Configuration* -> *Configure Module* and set *Module Type* to `Sonoff Pow R2`. The device will now restart.",
    },
    {
      image: {
        alt: "Tasmota GUI - Power",
        url: "https://lh3.googleusercontent.com/pw/ACtC-3coDFXthHtmsy-gwu0PpGWGy2Q6zlTJNAHKioT7zX102X0zXgTQ_bsGkHgc975Ig2lCXVo7YazlvQ4-A0ilTWyE8puvrPsg0Sary0LQW1x7r7u-jNrElDk-OSYsFw41wbTlG5FZjn3GHhkgV52OQIDH=w309-h455-no",
        avoidLazy: true,
        align: "left",
        style: {
          maxWidth: "25rem",
